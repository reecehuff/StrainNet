...
def load_model(args, model_type, train=False):
    ...
    # Load the model
    model = torch.load(os.path.join(args.model_dir, model_name + '.pt'))
    # Send the model to the device
    model.to(args.device)
     # Set the model to training or evaluation
    if train:
...
def save_model(model, args, model_type):
    ...
    torch.save(model, os.path.join(args.model_dir, model_name + '.pt'))
    torch.save(model, os.path.join(args.log_dir, model_name + '.pt'))

...
class UNet(nn.Module):
    def __init__(self, args):
        ...
        self.conv0_0 = Block(self.input_channels, nb_filter[0], nb_filter[0])
...
class DeformationClassifier(nn.Module):
    def __init__(self, args):
        ...
        self.CNN = nn.Sequential(
                        nn.Conv2d(in_channels=self.input_channels, out_channels=12, kernel_size=5, padding=1, stride=1, bias=True),
                        nn.BatchNorm2d(12),
                        nn.ReLU(),
                        nn.MaxPool2d(2,2),
    ...
    def forward(self, imgs):
        ...
        x = self.flatten(x)
        x = self.MLP(x)
        deformation_type = self.softmax(x)
